...
**Do not start Week 15 (trajectory / outcome eval) from this chapter** — this week ships a **small second agentic system that actually mutates state**, with A2A only where the peer is a different process/vendor. Trajectory scoring belongs next week. Do **not** skip this week for “the model promised it would be careful.” You cannot debug Week 15 trajectories if writes are implicit, duplicated, or unaudited.
...
**Syllabus artifact: a small second system that writes.** What you ship: one bounded agent (or graph) whose **success criterion is a mutated store**, plus the four controls. It can be a sibling of the Week 11–13 app (recommended) rather than a rewrite.
...
Read the concepts in order. Each section’s **Worked Example** and **Apply It** assume the same flagship service (working title: Deployment Copilot) gaining a **side-effecting specialist** with a safety envelope — optionally reachable over A2A.
...
- Four syllabus concepts match research files `01`–`04`; overview material from `00` is in chapter framing only (no extra `###` concept).  
- File `03` is taught as **one** envelope concept (idempotency + confirmation + dry-run), matching the research file boundary and “teach as a set” instruction.  
- `[NEEDS MORE RESEARCH]` markers appear under Apply It where research left open questions (A2A preview skills / idempotency metadata; dry-run over A2A skill surface; dynamic tool withholding; A2A key trust and confirmer identity; OTel A2A attributes; audit retention / authority / WORM vs Postgres; checkpoint-as-derived-audit). Open questions were not invented into answers.
